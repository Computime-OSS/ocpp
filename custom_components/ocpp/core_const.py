"""Platform-agnostic constants and settings for the OCPP integration.

This module has no dependency on Home Assistant. Use it from chargepoint.py,
ocppv16.py, ocppv201.py, and platform_adapter.py so the core remains portable.
HA-specific constants (e.g. unit mappings) live in const.py and are used only
by the Home Assistant adapter and HA platforms (sensor, number, config_flow).
"""
...
# -----------------------------------------------------------------------------
# Config keys (string literals; same values as homeassistant.const where used)
# -----------------------------------------------------------------------------
...
DEFAULT_SSL = False
DEFAULT_SSL_CERTFILE_PATH = str(pathlib.Path.cwd().joinpath("fullchain.pem"))
DEFAULT_SSL_KEYFILE_PATH = str(pathlib.Path.cwd().joinpath("privkey.pem"))
...
# OCPP unit strings (platform-agnostic)
...
class CentralSystemSettings:
    ...
    subprotocols: list = field(default_factory=lambda: list(DEFAULT_SUBPROTOCOLS))
